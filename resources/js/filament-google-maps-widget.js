...
export default function filamentGoogleMapsWidget({
  ...
  return {
    ...
    loadGMaps: function () {
      return new Promise((resolve, reject) => {
        ...
        script.src =
          ...
          "&map_id=" +
          this.config.mapId +
          "&callback=filamentGoogleMapsAsyncLoad";
      ...
      google.maps.event.addListener(this.map, "click", (event) => {
        if (!this.isMapDragging && document.fullscreenElement) {
          ...
          if (originalModalContainer) {
            const originalModalCancelButton =
              originalModalContainer.querySelector(
    ...
    createMarkerContent: async function (location) {
      ...
      if (location.icon && typeof location.icon === "object") {
        if (location.icon.hasOwnProperty("url")) {
          ...
          pinOptions = createPinOptions({
            glyph: iconContainer,
            scale: location.icon.scale,
          });
        } else if (
          location.icon.hasOwnProperty("glyphColor") ||
          location.icon.hasOwnProperty("borderColor") ||
          location.icon.type === "pin"
        ) {
          const pinScale =
            typeof location.icon.scale === "number" ? location.icon.scale : 1;
          pinOptions = createPinOptions({
            scale: pinScale,
      ...
      const pin =
        Object.keys(pinOptions).length > 0
          ? new PinElement(pinOptions)
          : undefined;
    ...
    createMarker: async function (location) {
      ...
      const headerContent = location.header ?? location.label ?? "";
    ...
    createHeaderContentNode: function (headerContent) {
      if (!headerContent) {
        return "";
      }
      ...
      template.innerHTML = headerContent.trim();
      return template.content.firstChild ?? headerContent;
    },
    applyOverlappingOffset: function () {
      const offsetDistance = parseFloat(
        this.config.overlappingMarkerOffset ?? 0
      );
      ...
      groups.forEach((entries, key) => {
        ...
        const center = originals.get(key);
        ...
        entries.forEach((entry, index) => {
          ...
          entry.location.lat = offsetPoint.lat();
          entry.location.lng = offsetPoint.lng();
        });
      });
    },
    createMarkerListener: function (marker) {
      const handleInfowindow = (marker) => {
        if (
          !this.infoWindow.get("closed") &&
          this.infoWindow.anchor === marker
        ) {
          ...
          // Attach a generic handler for forms rendered inside the InfoWindow
          google.maps.event.addListenerOnce(this.infoWindow, "domready", () => {
            try {
              // Try to scope to our content root if provided
              const scopeEl =
              ...
              const handler = (e) => {
                e.preventDefault();
                const method =
                  form.getAttribute("data-fgm-method") || "updateStopFromMap";
      ...
      if (this.config.markerAction && marker.model_id !== 0) {
        marker.addListener("click", () => {
          ...
          setTimeout(() => {
            this.$wire
              .mountAction(this.config.markerAction, {
                ...
                if (document.fullscreenElement) {
                  ...
                  if (modalContainer && document.fullscreenElement) {
                    ...
                    if (childElement) {
                      ...
                      const cancelButton = clonedModal.querySelector(
                        'button[x-on\\:click="close()"]'
                      );
                      const originalCancelButton = modalContainer.querySelector(
                        'button[x-on\\:click="close()"]'
                      );
    ...
    mergeMarkers: async function () {
      ...
      for (const marker of this.markers) {
        let location = this.data.find(
          (loc) =>
            (loc.location.lat === marker.position.lat &&
              loc.location.lng === marker.position.lng) ||
            (loc.origin?.location.lat === marker.position.lat &&
              loc.origin?.location.lng === marker.position.lng)
        );

        if (
          location?.origin?.location.lat === marker.position.lat &&
          location.origin.location.lng === marker.position.lng
        ) {
        ...
        if (location) {
          ...
          // Ensure InfoWindow content reflects latest data
          if (location.info && marker.info !== location.info) {
      ...
      const newLocations = this.data.filter(
        (loc) =>
          !updatedMarkers.some(
            (marker) =>
              marker.position.lat === loc.location.lat &&
              marker.position.lng === loc.location.lng
          )
      );
      ...
      for (const location of newLocations) {
        ...
        if (
          ...
          !updatedMarkers.some(
            (marker) =>
              marker.position.lat === location.origin.location.lat &&
              marker.position.lng === location.origin.location.lng
          )
        ) {
    ...
    createPolyline: function (groupedPolylineInfo) {
      ...
      if (groupedPolylineInfo.symbol) {
        ...
        for (let i = 0; i < polylineLengths.length; i++) {
          ...
          if (isNorthwards) {
            offsetPercentage =
              ((accumulatedLength - polylineLengths[i] * 0.2) /
                totalPolylineLength) *
              100;
          } else {
            offsetPercentage =
              ((accumulatedLength - polylineLengths[i] * 0.6) /
                totalPolylineLength) *
              100;
          }
      ...
      if (this.config.clustering) {
        google.maps.event.addListener(this.clusterer, "clusteringend", () => {
          ...
          Alpine.raw(this.polylines).forEach((polyline, index) => {
            ...
            for (const cluster of this.clusterer.clusters) {
              if (cluster.count > 1) {
                ...
                const isAnyMarkerOnPolyline = markers.some((marker) =>
                  polylineCoords.some(
                    (polylineCoord) =>
                      polylineCoord.lat === marker.position.lat &&
                      polylineCoord.lng === marker.position.lng
                  )
                );
            ...
            polyline.setMap(
              polylineShouldBeVisible ? Alpine.raw(this.map) : null
            );
          });
        });
      }
    ...
    hasPolylineChanged: function (oldPolyline, newPolylineInfo) {
      if (
        oldPolyline.strokeColor !== newPolylineInfo.strokeColor ||
        oldPolyline.strokeOpacity !== newPolylineInfo.strokeOpacity ||
        oldPolyline.strokeWeight !== newPolylineInfo.strokeWeight
      ) {
        return true;
      }
      const oldPolylineCoords = oldPolyline
        .getPath()
        .getArray()
        .map((p) => ({ lat: p.lat(), lng: p.lng() }));
      const newPolylineCoords = newPolylineInfo
        .getPath()
        .getArray()
        .map((p) => ({ lat: p.lat(), lng: p.lng() }));
      ...
      for (let i = 0; i < oldPolylineCoords.length; i++) {
        if (
          oldPolylineCoords[i].lat !== newPolylineCoords[i].lat ||
          oldPolylineCoords[i].lng !== newPolylineCoords[i].lng
        ) {
          return true;
        }
      }
